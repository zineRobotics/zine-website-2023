...
export const deleteProject = async (data: IProject) => {
    await deleteDoc(doc(userTasksCollection, data.id))
    const roomName = data.taskData.roomName || `${data.taskData.title.split(' ')[0]}-${data.usersData[0].email.slice(4).split('@')[0]}`
