...
const Channels = () => {
  ...
  const [currRoomID, setCurrRoomID] =
    useState<string>("");
  ...
  const [replyText, setReplyText] =
    useState<string>("");
  const [replyingName, setReplyingName] =
    useState<string>("");
  const [
    replyingMessageID,
    setReplyingMessageID,
  ] = useState<string | null>(null);
  const [currRoomImage, setCurrRoomImage] =
    useState<string>("");
  ...
  const fetchSubCollectionMessages = async (
    roomID: string
  ) => {
    try {
      ...
      const documents = messagesSnapshot.docs.map(
        (doc) => ({
          id: doc.id,
      ...
      console.error(
        "Error fetching subcollection documents:",
        error
      );
    }
  };
  const fetchRooms = async () => {
    const promises = authUser?.roomids.map(
      async (roomID: string) => {
        const roomRef = doc(db, "rooms", roomID);
  ...
  function RenderMessageWithLinks({
    ...
    return (
      ...
      >
        ...
        >
          <div
            className="whitespace-pre-wrap break-words pl-3 text-white text-sm w-fit"
            style={{
              cursor: "pointer",
              userSelect: "none",
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={`bg-white w-4/5 text-sm px-6 py-3 ${
              user ? "ml-auto" : ""
            }`}
            style={{
              borderRadius: `${
        ...
        <div className="flex h-screen md:col-span-9">
            ...
            !hide && (
              <div className="fixed bg-white h-full w-screen bottom-0 z-40 pt-20">
                <div className={`w-4/5 mx-auto`}>
                  ...
                  <div
                    className="font-normal w-3/5 mt-5"
                    style={{ color: "#8D989F" }}
                  >
                    Groups
                  </div>
          ...
          {screenWidth >= 768 ? (
            <div className="flex-1 bg-gray-100 flex flex-col sm:w-full">
              ...
              {currRoom !== "Announcements" &&
              currRoom !== null ? (
                <div className="flex rounded-xl mx-2 overflow-hidden border-2 mb-2 md:mb-2 mt-2 md:mx-4 bg-white">
                  <input
                    className="py-2 px-4 w-full outline-none bg-white"
                  ...
                  />
                  <div
                    className="bg-white pt-1 px-2 ml-1 mr-2 cursor-pointer my-auto"
                    onClick={() => {
                      handleSend();
                    }}
                  >
